...
export default function ExpertiseAdminPage() {
  ...
  async function addExpertise(e: React.FormEvent) {
    ...
    const { data, error } = await supabase
      .from("expertise")
      .insert([{ title: newExpertise.title, description: newExpertise.description, published: newExpertise.published }])
      .select();
    ...
    if (expertiseFile) {
      ...
      if (!uploadError && uploadData) {
        ...
        await supabase.from("expertise").update({ image: imageUrl }).eq("id", created.id);
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    if (!error && data?.[0]) {
      setExpertise(prev => prev.map(p => (p.id === editing.id ? data[0] : p)));
      setEditing(null);
    } else {
  ...
  async function handleExpertiseImageUpload(e: React.ChangeEvent<HTMLInputElement>, id: string) {
    ...
    const { data, error } = await supabase.storage.from("expertise-image").upload(fileName, file);

    if (!error && data) {
      const imageUrl = supabase.storage.from("expertise-image").getPublicUrl(fileName).data.publicUrl;
      await supabase.from("expertise").update({ image: imageUrl }).eq("id", id);
      setExpertise(prev => prev.map(p => (p.id === id ? { ...p, image: imageUrl } : p)));
    }
    ...
    <div className="admin-content">
      ...
      <form onSubmit={addExpertise} className="cms-add-form">
        ...
        <label className="cms-btn cms-btn--upload">
          <input type="file" accept="image/*" hidden onChange={e => setExpertiseFile(e.target.files?.[0] ?? null)} />
          {uploading ? "Uploaden..." : "Upload foto"}
        </label>

        <button type="submit" className="cms-btn cms-btn--primary">Toevoegen</button>
      </form>
  ...
  {expertise.map(item => (
    <div key={item.id} className="expertise-card">
      {item.image ? (
        <img src={item.image} alt={item.title} className="expertise-thumb" />
      ) : (
        ...
        <div className="cms-modal-overlay">
          <div className="cms-edit-container">
            ...
            <form onSubmit={saveEdit}>
              ...
              <textarea value={editForm.description} onChange={e => setEditForm({ ...editForm, description: e.target.value })} required className="cms-textarea-large"/>
